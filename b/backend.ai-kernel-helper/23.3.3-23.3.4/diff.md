# Comparing `tmp/backend.ai-kernel-helper-23.3.3.tar.gz` & `tmp/backend.ai-kernel-helper-23.3.4.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "backend.ai-kernel-helper-23.3.3.tar", last modified: Thu May 25 17:30:38 2023, max compression
+gzip compressed data, was "backend.ai-kernel-helper-23.3.4.tar", last modified: Mon May 29 10:42:35 2023, max compression
```

## Comparing `backend.ai-kernel-helper-23.3.3.tar` & `backend.ai-kernel-helper-23.3.4.tar`

### file list

```diff
@@ -1,20 +1,20 @@
-drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-25 17:30:38.165115 backend.ai-kernel-helper-23.3.3/
--rw-r--r--   0 runner    (1001) docker     (123)       12 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/MANIFEST.in
--rw-r--r--   0 runner    (1001) docker     (123)     1021 2023-05-25 17:30:38.165115 backend.ai-kernel-helper-23.3.3/PKG-INFO
-drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-25 17:30:38.161115 backend.ai-kernel-helper-23.3.3/ai/
-drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-25 17:30:38.161115 backend.ai-kernel-helper-23.3.3/ai/backend/
-drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-25 17:30:38.165115 backend.ai-kernel-helper-23.3.3/ai/backend/helpers/
--rw-r--r--   0 runner    (1001) docker     (123)        8 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/ai/backend/helpers/VERSION
--rw-r--r--   0 runner    (1001) docker     (123)      110 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/ai/backend/helpers/__init__.py
--rw-r--r--   0 runner    (1001) docker     (123)     1281 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/ai/backend/helpers/package.py
-drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-25 17:30:38.165115 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/
--rw-r--r--   0 runner    (1001) docker     (123)     1021 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/PKG-INFO
--rw-r--r--   0 runner    (1001) docker     (123)      467 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/SOURCES.txt
--rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/dependency_links.txt
--rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/namespace_packages.txt
--rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/not-zip-safe
--rw-r--r--   0 runner    (1001) docker     (123)       20 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/requires.txt
--rw-r--r--   0 runner    (1001) docker     (123)        3 2023-05-25 17:30:38.000000 backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/top_level.txt
--rw-r--r--   0 runner    (1001) docker     (123)      749 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/backend_shim.py
--rw-r--r--   0 runner    (1001) docker     (123)       38 2023-05-25 17:30:38.165115 backend.ai-kernel-helper-23.3.3/setup.cfg
--rw-r--r--   0 runner    (1001) docker     (123)     1562 2023-05-25 17:30:37.000000 backend.ai-kernel-helper-23.3.3/setup.py
+drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-29 10:42:35.110953 backend.ai-kernel-helper-23.3.4/
+-rw-r--r--   0 runner    (1001) docker     (123)       12 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/MANIFEST.in
+-rw-r--r--   0 runner    (1001) docker     (123)     1021 2023-05-29 10:42:35.110953 backend.ai-kernel-helper-23.3.4/PKG-INFO
+drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-29 10:42:35.106953 backend.ai-kernel-helper-23.3.4/ai/
+drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-29 10:42:35.106953 backend.ai-kernel-helper-23.3.4/ai/backend/
+drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-29 10:42:35.110953 backend.ai-kernel-helper-23.3.4/ai/backend/helpers/
+-rw-r--r--   0 runner    (1001) docker     (123)        8 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/ai/backend/helpers/VERSION
+-rw-r--r--   0 runner    (1001) docker     (123)      110 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/ai/backend/helpers/__init__.py
+-rw-r--r--   0 runner    (1001) docker     (123)     1281 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/ai/backend/helpers/package.py
+drwxr-xr-x   0 runner    (1001) docker     (123)        0 2023-05-29 10:42:35.110953 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/
+-rw-r--r--   0 runner    (1001) docker     (123)     1021 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/PKG-INFO
+-rw-r--r--   0 runner    (1001) docker     (123)      467 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/SOURCES.txt
+-rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/dependency_links.txt
+-rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/namespace_packages.txt
+-rw-r--r--   0 runner    (1001) docker     (123)        1 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/not-zip-safe
+-rw-r--r--   0 runner    (1001) docker     (123)       17 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/requires.txt
+-rw-r--r--   0 runner    (1001) docker     (123)        3 2023-05-29 10:42:35.000000 backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/top_level.txt
+-rw-r--r--   0 runner    (1001) docker     (123)      749 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/backend_shim.py
+-rw-r--r--   0 runner    (1001) docker     (123)       38 2023-05-29 10:42:35.110953 backend.ai-kernel-helper-23.3.4/setup.cfg
+-rw-r--r--   0 runner    (1001) docker     (123)     1559 2023-05-29 10:42:34.000000 backend.ai-kernel-helper-23.3.4/setup.py
```

### Comparing `backend.ai-kernel-helper-23.3.3/PKG-INFO` & `backend.ai-kernel-helper-23.3.4/PKG-INFO`

 * *Files 1% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: backend.ai-kernel-helper
-Version: 23.3.3
+Version: 23.3.4
 Summary: Backend.AI Kernel Runner Prebuilt Binaries Package
 Home-page: https://github.com/lablup/backend.ai
 Author: Lablup Inc. and contributors
 License: LGPLv3
 Project-URL: Documentation, https://docs.backend.ai/
 Project-URL: Source, https://github.com/lablup/backend.ai
 Classifier: Intended Audience :: Developers
```

### Comparing `backend.ai-kernel-helper-23.3.3/ai/backend/helpers/package.py` & `backend.ai-kernel-helper-23.3.4/ai/backend/helpers/package.py`

 * *Files identical despite different names*

### Comparing `backend.ai-kernel-helper-23.3.3/backend.ai_kernel_helper.egg-info/PKG-INFO` & `backend.ai-kernel-helper-23.3.4/backend.ai_kernel_helper.egg-info/PKG-INFO`

 * *Files 1% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: backend.ai-kernel-helper
-Version: 23.3.3
+Version: 23.3.4
 Summary: Backend.AI Kernel Runner Prebuilt Binaries Package
 Home-page: https://github.com/lablup/backend.ai
 Author: Lablup Inc. and contributors
 License: LGPLv3
 Project-URL: Documentation, https://docs.backend.ai/
 Project-URL: Source, https://github.com/lablup/backend.ai
 Classifier: Intended Audience :: Developers
```

### Comparing `backend.ai-kernel-helper-23.3.3/backend_shim.py` & `backend.ai-kernel-helper-23.3.4/backend_shim.py`

 * *Files identical despite different names*

### Comparing `backend.ai-kernel-helper-23.3.3/setup.py` & `backend.ai-kernel-helper-23.3.4/setup.py`

 * *Files 8% similar despite different names*

```diff
@@ -17,15 +17,15 @@
         'Topic :: Software Development',
         'Development Status :: 5 - Production/Stable',
         'Programming Language :: Python :: 3.11',
         'License :: OSI Approved :: GNU Lesser General Public License v3 or later (LGPLv3+)',
     ],
     'description': 'Backend.AI Kernel Runner Prebuilt Binaries Package',
     'install_requires': (
-        'types-pkg_resources',
+        'types-setuptools',
     ),
     'license': 'LGPLv3',
     'long_description': """# Backend.AI In-kernel Helper Package
 """,
     'long_description_content_type': 'text/markdown',
     'name': 'backend.ai-kernel-helper',
     'namespace_packages': (
@@ -40,11 +40,11 @@
     ),
     'project_urls': {
         'Documentation': 'https://docs.backend.ai/',
         'Source': 'https://github.com/lablup/backend.ai',
     },
     'python_requires': '>=3.11,<3.12',
     'url': 'https://github.com/lablup/backend.ai',
-    'version': """23.03.3
+    'version': """23.03.4
 """,
     'zip_safe': False,
 })
```

